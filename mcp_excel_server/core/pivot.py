...
def create_pivot_table(
    ...
    try:
        ...
        cleaned_columns = [clean_field_name(field) for field in columns] if columns else []
        ...
        if cleaned_columns:
            ...
            for combo in unique_col_combinations:
                for value_field in cleaned_values:
                    header = " | ".join([str(combo[c]) for c in cleaned_columns]) + f" | {value_field} ({agg_func})"
        ...
        total_cols = len(cleaned_rows) + (len(col_headers) if col_headers else len(cleaned_values))
        # Write data rows
        current_row = 2
